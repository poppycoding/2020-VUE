...
<template>
    <div>
        <div>
            <el-input class="positionAdd" placeholder="添加职位..." prefix-icon="el-icon-plus" size="small"
                      v-model="position.name"></el-input>
        ...
        <div>
            <el-table :data="data" @selection-change="handleSelectionChange"
                      border class="positionTable" stripe>
        ...
        <el-dialog :visible.sync="dialogVisible" title="修改职位" width="30%">
            <div>
                ...
                <el-input size="small" style="width: 200px; margin-left: 10px" v-model="updatePosition.name"></el-input>
            </div>
        ...
        methods: {
            ...
            handleSave() {
                if (this.position.name) {
                    this.postRequest("/sys/basic/positions", this.position).then(response => {
            ...
            handleDelete(index, row) {
                this.$confirm('删除职位: ' + row.name + ' 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/sys/basic/positions/" + row.id).then(response => {
                        if (response.code === 200) {
                            this.listPositions();
                        }
                    })
            ...
            // 编辑: 成功则加载数据,隐藏对话框
            handleEdit() {
                this.putRequest("/sys/basic/positions", this.updatePosition).then(response => {
                    if (response.code === 200) {
                        this.listPositions();
            ...
            handleBatchDelete() {
                this.$confirm('批量删除: ' + this.multiSelectRows.length + ' 条职位,是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
